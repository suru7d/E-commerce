...
const SustainableChat = ({ onApiCall }) => {
    ...
    setMessages([
      {
        role: "assistant",
        content:
          "Hello! I'm your sustainable shopping assistant. I can help you find eco-friendly products and answer questions about sustainable shopping practices. How can I assist you today?",
      },
    ]);
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      typingEffect.active &&
      typingEffect.text.length < typingEffect.fullText.length
    ) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setResourceMetrics((prev) => ({
        ...
        carbonFootprint:
          prev.carbonFootprint + calculateChatCarbon(totalTokens),
      }));
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I'm sorry, I'm having trouble connecting to the sustainability service. Please try again later or consider these general tips: choose products with recycled materials, opt for items with less packaging, and look for eco-certifications.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      ...
      <p>
        Ask about eco-friendly products, sustainability practices, or get
        personalized green shopping recommendations.
      </p>
          ...
          {messages.map((message, index) => (
            ...
            >
              <div className="message-content">
                ...
                {message.sustainabilityTips &&
                  message.sustainabilityTips.length > 0 && (
      ...
      >
        ...
        <p>
          This chat is powered by Llama 2, a locally-run AI model through
          Ollama, which uses significantly less energy than cloud-based
          alternatives. The model is optimized for sustainable shopping
          guidance.
        </p>
        <ul style={{ marginTop: "10px" }}>
          <li>
            Estimated tokens processed: {resourceMetrics.tokenCount.toFixed(0)}
          </li>
          <li>Total requests: {resourceMetrics.totalRequests}</li>
          <li>
            Approximate carbon footprint:{" "}
            {resourceMetrics.carbonFootprint.toFixed(2)}g CO2e
          </li>
          <li>
            Carbon comparison: This chat uses about{" "}
            {((resourceMetrics.carbonFootprint * 100) / 4.5).toFixed(1)}% of the
